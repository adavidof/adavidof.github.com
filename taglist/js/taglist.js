$(function() {
	...
	TagList.prototype.init = function() {
		...
		// selectorName
		this.listTagsDeleteTagSpanSelector = 'div > button:last-child';
	...
	TagList.prototype.constructHTML = function() {
		...
		var $mainNode = $(
			...
			tagsArrayHtml +
			'</div>' +
			'<div class="tag-list-tags-add">' +
			'<div class="col-xs-10">' +
			'<input class="form-control" placeholder="' + this.textNewTag + '">' +
			'</div>' +
			'<div class="col-xs-2">' +
			'<button class="btn btn-default">' + this.textNewTagButtonText + '</button>' +
			'</div>' +
			'</div>' +
			'</div>');
		return $mainNode;
	...
	TagList.prototype.tagToHtml = function(tag, option) {
		...
		var $tagHtml = $(
			...
			'<button class="btn btn-sample btn-xs tag-list-tags-delete' + ((!option) ? ' ' + this.listTagsDeleteTagClassName : '') + '">X</button>' +
			'</div>');
